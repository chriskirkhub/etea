...
#' features_vec <- etea_features(textColumn, termNum=4,verbose=TRUE,use_stopwords=TRUE)
#' features_vec
#'
...
if (convert_to_tm == "TRUE") {
    require("tm")
    matrix_words <- findFreqTerms(q_matrix,lowfreq=termNum)
    }
else {
    require("quanteda")
    freqTerms <- topfeatures(q_matrix, n = nfeature(doc)) #quanteda
